module.exports = {
        ...
        if(!message.member.hasPermission(['DEAFEN_MEMBERS', 'KICK_MEMBERS'])) {
            ...
            if(member) {
                if(member.hasPermission(['KICK_MEMBERS', 'DEAFEN_MEMBERS']) && !message.member.hasPermission('ADMINISTRATOR')) {
                    message.reply("Você não pode desmutar essa pessoa! Ela é ou mais poderosa que você ou tem as mesmas permissões.")
                }
                else {
                    let mutedRole = message.guild.roles.cache.find(x => x.name === "Silenciado")
                    if(mutedRole) {
                        member.roles.remove(mutedRole)
                        message.reply("Membro foi desmutado!")
                    }
                    if(!mutedRole) {
    ...
    aliases: ['desmutar'],
    description: 'Desmuta um membro pelo seu id!'
}
